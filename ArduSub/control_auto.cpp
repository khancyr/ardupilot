...
void Sub::auto_circle_movetoedge_start(const Location_Class &circle_center, float radius_m)
{
    ...
    if (!circle_center.get_vector_from_origin_NEU(circle_center_neu)) {
        ...
        ahrs.get_relative_position_NEU_origin_cm(circle_center_neu);
    ...
    const float dist_to_edge = (curr_pos - circle_edge_neu).length();
    ...
    if (dist_to_edge > 300.0f) {
        ...
        const float dist_to_center = norm(circle_center_neu.x - curr_pos.x, circle_center_neu.y - curr_pos.y);
...
bool Sub::auto_loiter_start()
{
    ...
    Vector3f origin;
    ahrs.get_relative_position_NEU_origin_cm(origin);
