    ...
    */
    ...
    if (!motors->armed() && motors->get_interlock()) {
        ...
        if (g2.throw_type == ThrowType_Drop) {
            pos_control->set_alt_target(_copter.current_pos.z - 100.0f);
        } else {
            pos_control->set_alt_target(_copter.current_pos.z + 300.0f);
        }
    ...
    if ((stage != prev_stage) || (now - last_log_ms) > 100) {
        prev_stage = stage;
        last_log_ms = now;
        const float velocity = copter.current_vel.length();
        const float velocity_z = copter.current_vel.z;
