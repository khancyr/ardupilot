...
void Copter::poshold_run()
{
    ...
    if (ap.land_complete) {
        ...
        if (poshold.roll_mode == POSHOLD_BRAKE_READY_TO_LOITER && poshold.pitch_mode == POSHOLD_BRAKE_READY_TO_LOITER) {
            ...
            wp_nav->init_loiter_target(curr_pos, poshold.loiter_reset_I); // (false) to avoid I_term reset. In original code, velocity(0,0,0) was used instead of current velocity: wp_nav->init_loiter_target(inertial_nav.get_position(), Vector3f(0,0,0));
...
void Copter::poshold_update_wind_comp_estimate()
{
    ...
    Vector3f vel;
    ahrs.get_velocity_NEU_cm(vel);
    if (norm(vel.x, vel.y) > POSHOLD_WIND_COMP_ESTIMATE_SPEED_MAX) {
        return;
    }
